...
def apply_gang_changes(state, result):
    ...
    state["current_gang"] = normalize_gang(
        gang_data
    )
...
def load_game_file(uploaded_file):
    ...
    try:
        ...
        game["current_gang"] = normalize_gang(
            game.get(
                "current_gang",
                {
                    "name": "",
...
if current_gang.get("name"):
    ...
    gang_rank = current_gang.get(
        "rank",
        "成員"
    )

    gang_status = current_gang.get(
        "status",
        "正常"
    )
...
with st.expander("🏴 目前所屬幫派"):
    ...
    if not current_gang.get("name"):
        ...
        st.write(
            f"**目前身分：** "
            f"{current_gang['rank'] or '成員'}"
        )

        st.write(
            f"**加入時間：** "
            f"{current_gang['joined_at'] or '未知'}"
        )

        st.write(
            f"**目前狀態：** "
            f"{current_gang['status'] or '正常'}"
        )

        if current_gang.get("notes"):

            st.caption(
                f"幫派記憶："
                f"{current_gang['notes']}"
            )
